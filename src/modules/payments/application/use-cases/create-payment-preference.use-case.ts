...
import { PAYMENT_REPOSITORY, type IPaymentRepository } from '../../domain/interfaces/payment-repository.interface.js';
import { PAYMENT_PROVIDER, type IPaymentProvider } from '../../domain/interfaces/payment-provider.interface.js';
...
export class CreatePaymentPreferenceUseCase {
  ...
  async execute(order: {
    ...
    const { preferenceId, paymentUrl } = await this.paymentProvider.createPreference(order);
