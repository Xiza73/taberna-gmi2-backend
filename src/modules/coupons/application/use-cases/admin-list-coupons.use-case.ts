...
import { PaginatedResponseDto, PaginationDto } from '@shared/application/dtos/pagination.dto.js';
...
export class AdminListCouponsUseCase {
  ...
  async execute(query: PaginationDto): Promise<PaginatedResponseDto<CouponResponseDto>> {
    const page = query.page ?? 1;
    const limit = query.limit ?? 20;

    const { items, total } = await this.couponRepository.findAll({ page, limit });
